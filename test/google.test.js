...
describe("#### Google ####", function () {
    ...
    it("Cannot validate google in-app-purchase with a receipt.data that is not a string", function (done) {
        ...
        iap.config({ verbose: true, googlePublicKeyPath: fixedPkPath });

        iap.setup(function (error) {
            assert.equal(error, undefined);
            fs.readFile(fixedPath, function (error, data) {
                assert.equal(error, undefined);
    ...
    it("[CALLBACK] Can validate google in-app-purchase w/ auto-service detection", function (done) {
        // var path = process.argv[process.argv.length - 2].replace("--path=", "");
        // var pkPath = process.argv[process.argv.length - 1].replace("--pk=", "");
        ...
        iap.setup(async function (error) {
            assert.equal(error, undefined);
            const receiptJsonStr = await fsPromises.readFile(fixedPath, {
                encoding: "utf-8",
            });
            var receipt = JSON.parse(receiptJsonStr.toString());
            ...
            iap.validate(receipt, function (error, response) {
                ...
                for (var i = 0, len = data.length; i < len; i++) {
                    console.log("parsed purchased data", i, data[i]);
                    assert(data[i].productId);
                    // assert(data[i].transactionId); TODO: why undefined
                    assert(data[i].purchaseDate);
                    assert(data[i].quantity);
                }
                done();
    ...
    it("[PROMISE] Can validate google in-app-purchase w/ auto-service detection", async () => {
        var iap = require("../");
        ...
        var receiptToValidate = JSON.parse(receiptJsonStr.toString());

        const response = await iap.validate(receiptToValidate);
    ...
    it("Can validate google in-app-purchase w/ Promise & auto service detection", function (done) {
        ...
        iap.config({
            verbose: true,
            googlePublicKeyPath: fixedPkPath,
        });
        var receipt = JSON.parse(fs.readFileSync(fixedPath, "utf8"));
    ...
    it("Can NOT validate google in-app-purchase with incorrect receipt w/ auto-service detection", function (done) {
        ...
        iap.setup(function (error) {
            assert.equal(error, undefined);
            iap.validate(
                { data: "fake-receipt", signature: "fake" },
    ...
    it("[PROMISE] Can validate google in-app-purchase and check subscription state and fail", async () => {
        var iap = require("../");
        ...
        const receipt = JSON.parse(receiptJsonStr);

        try {
            await iap.validate(iap.GOOGLE, receipt);
        } catch (err) {
            assert.equal(typeof err, "string");
            assert.equal(
                err,
                '{"error":{},"status":1,"message":"{\\"error\\":\\"invalid_client\\",\\"error_description\\":\\"The OAuth client was not found.\\"}"}'
            );
        }
    });
    ...
    it.skip('Can validate google in-app-purchase with public key as string "googlePublicKeyStrLive"', function (done) {
                    ...
                    fs.readFile(path, function (error, data) {
                        ...
                        iap.validate(
                            ...
                            function (error, response) {
                                ...
                                exec(
                                    "unset GOOGLE_IAB_PUBLICKEY_SANDBOX",
                                    done
                                );
                            }
                        );
    ...
    it.skip("Can validate google in-app-purchase with public key from ENV variable", function (done) {
        var exec = require("child_process").exec;
        var path = process.argv[process.argv.length - 2].replace("--path=", "");
        var pkPath = process.argv[process.argv.length - 1].replace("--pk=", "");
    ...
    * Providing subscription info *
    ...
    it.skip("Access to subscription even info has an invalid access token", function (done) {
        // var path = process.argv[process.argv.length - 2].replace("--path=", "");
        // var pkPath = process.argv[process.argv.length - 1].replace("--pk=", "");
    ...
    it.skip("Cannot refresh access due to an invalid refresh token", function (done) {
        // var path = process.argv[process.argv.length - 2].replace("--path=", "");
        // var pkPath = process.argv[process.argv.length - 1].replace("--pk=", "");
        // var api = process.argv[process.argv.length - 3].replace("--api=", "");
    ...
    it.skip("Cannot call refresh access function if did not provide needed information", function (done) {
        var path = process.argv[process.argv.length - 2].replace("--path=", "");
        var pkPath = process.argv[process.argv.length - 1].replace("--pk=", "");
        var api = process.argv[process.argv.length - 3].replace("--api=", "");
    ...
    // it.only("Can validate Unity google in-app-purchase with dynamically fed public key", function (done) {
    //     var exec = require("child_process").exec;
    //     var path = process.cwd() + "/test/receipts/unity_google";
    //     var pkPath = process.argv[process.argv.length - 1].replace("--pk=", "");
    ...
    it("Can validate google in-app-purchase with dynamically fed public key", function (done) {
        ...
        iap.reset();
        fs.readFile(
            fixedPkPath + "iap-live",
            "utf-8",
            function (error, pkeyValue) {
                assert.equal(error, undefined);
                iap.config({
                    verbose: true,
                });
