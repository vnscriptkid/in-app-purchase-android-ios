...
module.exports.validatePurchase = function (secret, receipt, cb) {
    ...
    if (config && config.appleExcludeOldTransactions) {
        content["exclude-old-transactions"] =
            config.appleExcludeOldTransactions;
    }
    ...
    var tryProd = function (next) {
        ...
        send(prodPath, content, function (error, res, data) {
            ...
            validatedData.sandbox = false;
            verbose.log(
                "<Apple> Production validation successful:",
                validatedData
            );
            isValid = true;
            next();
    ...
    var trySandbox = function (next) {
        ...
        send(sandboxPath, content, function (error, res, data) {
            ...
            validatedData.sandbox = true;
            verbose.log(
                "<Apple> Sandbox validation successful:",
                validatedData
            );
            next();
        });
        ...
        */
        ...
        for (var i = 0, len = list.length; i < len; i++) {
            ...
            if (
                ...
                parsed.originalTransactionId =
                    parsed.originalTransactionId.toString();
            }
            ...
            parsed.bundleId =
                purchase.receipt[REC_KEYS.BUNDLE_ID] ||
                purchase.receipt[REC_KEYS.BID];
...
function handleResponse(receipt, data, cb) {
    ...
    if (data.status === constants.VALIDATION.SUCCESS) {
        if (
            ...
            return cb(
                new Error("failed to validate for empty purchased list"),
                data
            );
        }
