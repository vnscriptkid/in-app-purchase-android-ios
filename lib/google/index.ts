...
let config: any = null;
let keyPathMap: any = {};
let publicKeyMap: any = {};
let googleTokenMap: any = {};
...
module.exports.readConfig = function (configIn: any) {
  if (!configIn) {
...
module.exports.setup = function (cb: any) {
  ...
  if (!config || !config.googlePublicKeyPath) {
    ...
    if (process.env[ENV_PUBLICKEY.SANDBOX]) {
      publicKeyMap.sandbox = process.env[ENV_PUBLICKEY.SANDBOX]?.replace(
        /s+$/,
        ""
      );
    }
  ...
  async.eachSeries(
    keys,
    function (key: any, next: any) {
      var pkeyPath = keyPathMap[key];
      fs.readFile(pkeyPath, function (error: any, fileData: any) {
 ...
 */
module.exports.validatePurchase = function (
  dPubkey: any,
  receipt: any,
  cb: any
) {
  ...
  if (dPubkey && typeof dPubkey === "object") {
    if (dPubkey.clientId && dPubkey.clientSecret && dPubkey.refreshToken) {
      verbose.log(
        NAME,
        "Using dynamically fed google client ID, client secret, and refresh token:",
        dPubkey
      );
      // override tokenMap
  ...
  if (tokenMap.clientId && tokenMap.clientSecret && tokenMap.refreshToken) {
    ...
    validateMethod = function (_cb: any) {
  ...
  if (testMode) {
    ...
    validateMethod(function (error: any, data: any) {
      ...
      verbose.log(
        NAME,
        "Validation against sandbox public key successful:",
        data
      );
      // sandbox worked
  ...
  validateMethod(function (error: any, data: any) {
    if (error) {
      ...
      receipt.data = JSON.stringify(receipt.data).replace(/\//g, "\\/");
      validateMethod(function (error2: any, data: any) {
        if (error2) {
...
function isSubscription(receipt: any) {
...
function validateProduct(receipt: any, tokenMap: any, cb: any) {
  // if the receipt is a subscription, we validate w/ Google API instead
  // checkSubscriptionStatus() is most likely unnecessary now
  if (isSubscription(receipt)) {
    return validateSubscription(receipt, tokenMap, cb);
  ...
  if (!tokenMap.clientId || !tokenMap.clientSecret || !tokenMap.refreshToken) {
    return cb(
      new Error("missing google play api info"),
      createErrorData(receipt, {
        status: constants.VALIDATION.FAILURE,
        message:
          "client_id, client_secret, access_token and refres_token should be provided",
      })
    );
  }
  auth(tokenMap, function (error: any, body: any) {
    if (error) {
      return cb(
        error,
        createErrorData(receipt, {
    ...
    var url =
      ...
      "/tokens/" +
      encodeURIComponent(receipt.data.purchaseToken) +
      "?access_token=" +
  ...
  function _onProductValidate(error: any, res: any, body: any) {
    if (error) {
      return cb(
        error,
        createErrorData(receipt, {
    ...
    if (res.statusCode === 410) {
      ...
      return cb(
        new Error("ReceiptNoLongerValid"),
        createErrorData(receipt, {
          status: constants.VALIDATION.FAILURE,
          message: body,
        })
...
function validateSubscription(receipt: any, tokenMap: any, cb: any) {
  if (typeof receipt === "string") {
    try {
      receipt = JSON.parse(receipt);
    } catch (error: any) {
  ...
  auth(tokenMap, function (error: any, res: any) {
    if (error) {
      return cb(
        error,
        createErrorData(receipt, {
    ...
    if (!accessToken) {
      return cb(
        new Error(JSON.stringify(res.body || res)),
        createErrorData(receipt, {
          status: constants.VALIDATION.FAILURE,
          message: "failed to authenticate api call",
        })
  ...
  function _onSubscriptionValidate(error: any, res: any, body: any) {
    if (error) {
      return cb(
        error,
        createErrorData(receipt, {
...
module.exports.getPurchaseData = function (purchase: any, options: any) {
  if (!purchase) {
    return null;
  }
 ...
 */
function checkSubscriptionStatus(data: any, cb: any) {
  ...
  var urlPurchaseTypeSegment =
    data.autoRenewing === undefined ? "products" : "subscriptions";

  var url =
    "https://www.googleapis.com/androidpublisher/v3/applications/" +
    packageName +
    "/purchases/" +
    urlPurchaseTypeSegment +
    "/" +
    subscriptionID +
    "/tokens/" +
    purchaseToken;
  ...
  var getSubInfo = function (next: any) {
    verbose.log(NAME, "Get subscription info from", url);

    getSubscriptionInfo(url, function (error: any, response: any, body: any) {
      if (error || "error" in body) {
        verbose.log(
          NAME,
          "Failed to get subscription info from",
          url,
          error,
          body
        );
      ...
      verbose.log(
        NAME,
        "Successfully retrieved subscription info from",
        url,
        data
      );
  ...
  var validate = function (next: any) {
    switch (state) {
      ...
      case constants.VALIDATION.FAILURE:
        ...
        refreshGoogleTokens(function (error: any, res: any, body: any) {
          if (error) {
  ...
  var recheck = function (next: any) {
    if (state === constants.VALIDATION.SUCCESS) {
      ...
      getSubscriptionInfo(url, function (error: any, response: any, body: any) {
        ...
        verbose.log(
          NAME,
          "Re-check successfully retrieved subscription info:",
          url,
          data
        );
  ...
  var tasks = [getSubInfo, validate, recheck];
...
/** Deprecated as of Aug 1st 20017 b/c we now have validateSubscription()
 */
function _copyPurchaseDetails(data: any, body: any) {
...
function getPublicKey(publicKey: any) {
  if (!publicKey) {
    return null;
  }
...
function validatePublicKey(receipt: any, pkey: any, cb: any) {
...
function chunkSplit(str: any, len: any, end: any) {
...
function getSubscriptionInfo(url: any, cb: any) {
  var options = {
    method: "GET",
    url: url,
    headers: {
...
module.exports.refreshToken = function (cb: any) {
  if (!checkSubscriptionState) {
...
function refreshGoogleTokens(cb: any) {
  const body: any = {};
...
// redundant..... :(
function auth(tokenMap: any, cb: any) {
  const body: any = {};
  body[KEYS.GRANT_TYPE] = KEYS.REFRESH_TOKEN;
...
function createErrorData(receipt: any, obj: any) {
